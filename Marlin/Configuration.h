 ...
 */
  ...
  #define MPC_BLOCK_HEAT_CAPACITY { 15.61f }           // (J/K) Heat block heat capacities.
  #define MPC_SENSOR_RESPONSIVENESS { 0.2007f }         // (K/s per ∆K) Rate of change of sensor temperature from heat block.
  #define MPC_AMBIENT_XFER_COEFF { 0.1189f }           // (W/K) Heat transfer coefficients from heat block to room air with fan off.
  #if ENABLED(MPC_INCLUDE_FAN)
    #define MPC_AMBIENT_XFER_COEFF_FAN255 { 0.1469f }  // (W/K) Heat transfer coefficients from heat block to room air with fan on full.
  #endif
 ...
 */
#define DEFAULT_AXIS_STEPS_PER_UNIT   { 80, 80, 400, 409.58066 }
 ...
 *     O-- FRONT --+
 */
#define NOZZLE_TO_PROBE_OFFSET { -48, -10, -1.7 } // (mm) X, Y, Z distance from Nozzle tip to Probe trigger-point
